...
class ImportInitialFisierExcelController extends Controller
{
    ...
    public function importFirmeVerificaDupaCNP()
    {
        $salariati_import = DB::table('import_salariati')->get();

        echo '<h3>CNP Incorect:</h3><br>';
        $count = 1;
        foreach ($salariati_import as $salariat_import){
            if (!is_numeric($salariat_import->G)){
                echo $count++ . ' ' . $salariat_import->G . '<br>';
            }
        }

        echo '<h3>CNP Corect:</h3>';
        $count = 1;
        foreach ($salariati_import as $salariat_import){
            if (is_numeric($salariat_import->G)){
                echo $count++ . ' ' . $salariat_import->G . '<br>';
    ...
    public function importSalariati()
    {
        ...
        // Se creeaza intai firmele: randurile fara CNP
        $count = 1;
        foreach ($salariati_import as $salariat_import){
            if (!is_numeric($salariat_import->G)){ // Daca nu exista CNP, inseamna ca nu este salariat acolo
                if ($salariat_import->B){ // Daca este ceva in coloana B, inseamna ca este firma
                    $firma = new Firma();
                    $firma->nume = $salariat_import->B;
                    if(strpos($salariat_import->C, 'revisal') !== false){
                        echo $count++ . ' ' . $salariat_import->C . '<br>';
                    }
                }
            }
        }
